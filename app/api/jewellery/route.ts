...
export async function GET(request: NextRequest) {
  try {
    ...
    if (id) {
      const item = await jewelleryRepository.findById(id)
      ...
      const product = {
        ...
        images: item.images || [],
        is_month_highlight: (item as any).is_month_highlight ?? false,
        category: 'Jewellery',
    ...
    let list
    if (search) {
      list = await jewelleryRepository.searchJewellery(search, limit)
    } else if (minPrice || maxPrice) {
      list = await jewelleryRepository.findJewelleryWithFilters(
        {
          minPrice,
          maxPrice,
          isActive: true
      ...
      const all = await jewelleryRepository.findAll(limit, offset)
